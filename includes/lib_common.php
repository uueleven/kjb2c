...
//mod by ecshope.com ray soft inc
function curl_grab($url, $post = null, $retries = 3)
{
...
//mod by ecshope.com ray soft inc
function kjb2c_record_user($uname) 
{
	$url = 'http://trainer.kjb2c.com/reg/reg!eplatformEdit.do';
	$ts = local_date('Y-m-d H:i:s', gmtime() );
	$str = curl_grab($url, array('userid'=>$GLOBALS['_CFG']['kjb2c_id'], 'timestamp'=>$ts, 'sign'=>md5($GLOBALS['_CFG']['kjb2c_id'].$GLOBALS['_CFG']['kjb2c_secret'].$ts), 'rurl'=>'rurl', 'purl'=>'purl', 'account'=>$uname) );
	print($str);
	die();
}

//mod by ecshope.com ray soft inc
//$type 可选  order pay invoice
function kjb2c_send_order_ex($oid, $type='order', &$res_msg_arr=array() ) 
{
	$oinfo = order_info($oid);
	$ts = local_date('Y-m-d H:i:s', gmtime() );
	$uname = $GLOBALS['db']->getOne("select user_name from ".$GLOBALS['ecs']->table("users")." where user_id='".$oinfo['user_id']."'");
    $xml = new DomDocument('1.0', 'utf-8');
	$root = $xml->createElement('Message');
	$header = $xml->createElement('Header');
	$body = $xml->createElement('Body');
	$header->appendChild($xml->createElement('CustomsCode', $GLOBALS['_CFG']['kjb2c_custom_code']) );
	$header->appendChild($xml->createElement('OrgName', $GLOBALS['_CFG']['kjb2c_org_name']) );
	$header->appendChild($xml->createElement('CreateTime', $ts) );

	//mod by ecshope.com ray soft inc
	if('order' == $type)
	{
	$post_url = 'http://i.trainer.kjb2c.com/msg/ordermsg.do';
	$xml_order = $xml->createElement('Order');
	$xml_order->appendChild($xml->createElement('OrderNo', $oinfo['order_sn']) );
	$xml_order->appendChild($xml->createElement('GoodsAmount', $oinfo['total_fee']) );
	$xml_order->appendChild($xml->createElement('PostFee', $oinfo['shipping_fee']) );
	$xml_order->appendChild($xml->createElement('Amount', $oinfo['total_fee']) );
	$xml_order->appendChild($xml->createElement('BuyerAccount', $uname) );
	$xml_order->appendChild($xml->createElement('Promotion', '') );
	$xml_order_goods= $xml->createElement('Goods');
	$og_list = $GLOBALS['db']->getAll("select goods_price,goods_number,goods_sn,goods_name,goods_price*goods_number as subtotal from ".$GLOBALS['ecs']->table("order_goods")." where order_id='$oid'");
	foreach($og_list as $k=>$row) 
	{
		$xml_goods_detail = $xml->createElement('Detail');
		$xml_goods_detail->appendChild($xml->createElement('ProductId', $row['goods_sn']) );
		$xml_goods_detail->appendChild($xml->createElement('GoodsName', $row['goods_name']) );
		$xml_goods_detail->appendChild($xml->createElement('Qty', $row['goods_number']) );
		$xml_goods_detail->appendChild($xml->createElement('Unit', '件') );
		$xml_goods_detail->appendChild($xml->createElement('Price', $row['goods_price']) );
		$xml_goods_detail->appendChild($xml->createElement('Amount', $row['subtotal']) );
		$xml_order_goods->appendChild($xml_goods_detail);
	}
	$xml_order->appendChild($xml_order_goods);
	$body->appendChild($xml_order);
	...
	elseif('pay' == $type)
	{
	$post_url = 'http://i.trainer.kjb2c.com/msg/paymsg.do';
	$xml_order = $xml->createElement('Pay');
	$xml_order->appendChild($xml->createElement('PaymentNo', $oinfo['order_id']) );
	$xml_order->appendChild($xml->createElement('OrderNo', $oinfo['order_sn']) );
	$xml_order->appendChild($xml->createElement('OrderSeqNo', $oinfo['order_sn']) );
	$xml_order->appendChild($xml->createElement('Amount', $oinfo['total_fee']) );
	$xml_order->appendChild($xml->createElement('CurrCode', 'RMB') );
	$xml_order->appendChild($xml->createElement('BuyerAccount', $uname) );
	$body->appendChild($xml_order);
	$root->appendChild($header);
	$root->appendChild($body);
	...
	elseif('invoice' == $type)
	{
	$post_url = 'http://i.trainer.kjb2c.com/msg/logisticsmsg.do';
	$xml_order = $xml->createElement('Logistics');
	$oinfo['province_str'] = $GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table("region")." where region_id='".$oinfo['province']."'");
	$oinfo['city_str'] = $GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table("region")." where region_id='".$oinfo['city']."'");
	$oinfo['district_str'] = $GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table("region")." where region_id='".$oinfo['district']."'");
	$xml_order->appendChild($xml->createElement('LogisticsNo', $oinfo['invoice_no']) );
	$xml_order->appendChild($xml->createElement('OrderNo', $oinfo['order_sn']) );
	$xml_order->appendChild($xml->createElement('LogisticsName', $oinfo['shipping_name']) );
	$xml_order->appendChild($xml->createElement('Consignee', $oinfo['consignee']) );
	$xml_order->appendChild($xml->createElement('Province', $oinfo['province_str']) );
	$xml_order->appendChild($xml->createElement('City', $oinfo['city_str']) );
	$xml_order->appendChild($xml->createElement('District', $oinfo['district_str']) );
	$xml_order->appendChild($xml->createElement('ConsigneeAddr', sprintf("%s %s %s %s", $oinfo['province_str'], $oinfo['city_str'], $oinfo['district_str'], $oinfo['address']) ) );
	$xml_order->appendChild($xml->createElement('ConsigneeTel', $oinfo['tel']) );
	$xml_order->appendChild($xml->createElement('GoodsName', implode(",", $GLOBALS['db']->getCol("select goods_name from ".$GLOBALS['ecs']->table("order_goods")." where order_id='$oid'") ) ) );
	$body->appendChild($xml_order);
	$root->appendChild($header);
	$root->appendChild($body);
	$xml->appendChild($root);
	...
	$res_str = curl_grab($post_url, array('userid'=>$GLOBALS['_CFG']['kjb2c_id'], 'timestamp'=>$ts, 'sign'=>md5($GLOBALS['_CFG']['kjb2c_id'].$GLOBALS['_CFG']['kjb2c_secret'].$ts), 'xmlstr'=>$str) );

	$dom = new DomDocument('1.0', 'utf-8');
    $dom->loadXML($res_str);
	$node =$dom->getElementsByTagName('Result')->item(0);
	$result = $node->nodeValue;
	$node = $dom->getElementsByTagName('ResultMsg')->item(0);
	$result_msg = $node->nodeValue;
	$res_msg_arr[] = $result_msg;
	return $result=='T';
}

//mod by ecshope.com ray soft inc
function kjb2c_send_order($oid) 
{
	$oinfo = order_info($oid);
	if(!empty($oinfo) && PS_PAYED == $oinfo['pay_status'] && !$oinfo['kjb2c_sent'])
	{
	if(kjb2c_send_order_ex($oid, 'order', $msg_arr) && kjb2c_send_order_ex($oid, 'pay', $msg_arr) && kjb2c_send_order_ex($oid, 'invoice', $msg_arr) )
	{
	$GLOBALS['db']->query("update ".$GLOBALS['ecs']->table("order_info")." set kjb2c_sent='1' where order_id='$oid'");
	}
	print_r($msg_arr);
	}
}
...
//mod by ecshope.com ray soft inc
//isauth是否已认证（0=未认证 1=认证通过 2=认证未通过）							
function kjb2c_user_status($uname, &$isauth) 
{
	$ts = local_date('Y-m-d H:i:s', gmtime() );
	$xml = new DomDocument('1.0', 'utf-8');
	$root = $xml->createElement('Message');
	$header = $xml->createElement('Header');
	$header->appendChild($xml->createElement('Account', $uname) );
	$root->appendChild($header);
	$xml->appendChild($root);
	$str = $xml->saveXML();
	$res_str = curl_grab('http://i.trainer.kjb2c.com/msg/getconsumer.do', array('userid'=>$GLOBALS['_CFG']['kjb2c_id'], 'timestamp'=>$ts, 'sign'=>md5($GLOBALS['_CFG']['kjb2c_id'].$GLOBALS['_CFG']['kjb2c_secret'].$ts), 'xmlstr'=>$str) );
	$dom = new DomDocument('1.0', 'utf-8');
	$dom->loadXML($res_str);
	$node = $dom->getElementsByTagName('Result')->item(0);
	$result = $node->nodeValue;
	$isauth = $dom->getElementsByTagName('IsAuth')->item(0)->nodeValue;
	return intval($result=='T');
}
