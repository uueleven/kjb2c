...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated, kjb2c_binded FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";//mod by ecshope.com ray soft inc
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
	...
	$info['kjb2c_binded'] = kjb2c_user_status($_SESSION['user_name'], $is_auth);
	$info['kjb2c_is_auth'] = $is_auth;
