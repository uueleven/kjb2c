...
function proc_unserialize($match) 
{
    ...
    function load_session()
    {
        ...
        else
        {
            if (!empty($session['data']) && $this->_time - $session['expiry'] <= $this->max_life_time)
            {
				...
				$bad_data = $session['data'];
				$fixed_data = preg_replace_callback ( '!s:(\d+):"(.*?)";!', 'proc_unserialize',$bad_data );

                $GLOBALS['_SESSION']  = unserialize($fixed_data);
